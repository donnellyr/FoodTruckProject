	...
	private void userInput() {
		...
		for(int i = 0; i < truckList.length; i++) {
			...
			if(truckName.equals("Quit") || truckName.equals("quit")) {
				...
				++truckID;
				truckList[i] = "Truck ID: " + truckID + " Truck Name: " + truckName + " Food Type: " + foodType + " Rating: " + truckRating;
				average += truckRating;
		...
		while(repeat){
			...
			choice = sc.nextInt();
			switch(choice) {
			case 1: output.getfoodTruck(truckList);
			//appMenue();
			break;
			case 2: System.out.println("Average truck Rating: " + output.truckAverage(average, truckID));
			//appMenue();
			break;
			case 3: output.bestTruck(truckList, winner);
			//appMenue();
			break;
			case 4: System.out.println("Have a good day!"); repeat = false;
			break;
